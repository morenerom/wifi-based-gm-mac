...
public:
  ...
  WifiMacGmType GetCtrlPktType (void) const;
  void SetCtrlPktType (WifiMacGmType type);

  NodeType GetNodeType (void) const;
  void SetNodeType (NodeType type);

  int8_t GetGroupNumber (void) const;
  void SetGroupNumber(int8_t groupNumber);

  uint16_t GetFrameSize (void) const;
  void SetFrameSize (uint16_t frameSize);

  uint8_t GetTA (void) const;
  void SetTA (uint8_t TA);

  bool IsGmInit (void) const;
  bool IsGmData (void) const;

  uint32_t GetGmControl (void) const;
  void SetGmControl (uint32_t ctrl);
...
private:
  ...
  uint8_t m_ctrlPktType;  ///< GM type (4bits)
  uint8_t m_nodeType;  ///< the type of node (2bits)
  int8_t m_groupNumber; ///< group number (4bits)
  uint16_t m_frameSize; ///< frame size milliseconds (10bits)
  uint8_t m_TA; ///< TA milliseconds (6bits)
};
