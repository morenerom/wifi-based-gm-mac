...
// Adding for GM-MAC
int8_t
WifiMacHeader::GetGroupNumber(void) const
{
  return m_groupNumber;
}
void
WifiMacHeader::SetGroupNumber(int8_t groupNumber)
{
  m_groupNumber = groupNumber;
}

WifiMacGmType WifiMacHeader::GetCtrlPktType (void) const {
  switch(m_ctrlPktType) {
    case 0:
      return WIFI_MAC_GM_INIT;
    case 1:
      return WIFI_MAC_GM_DATA;
    case 2:
      return WIFI_MAC_GM_REQUEST;
    case 3:
      return WIFI_MAC_GM_REPLY;
  }
  // NOTREACHED
  NS_ASSERT (false);
  return (WifiMacGmType) - 1;
}
...
NodeType WifiMacHeader::GetNodeType (void) const {
  switch (m_nodeType) {
    case 0:
      return SINK;
    case 1:
...
void WifiMacHeader::SetNodeType (NodeType nodeTypeH) {
  m_nodeType = nodeTypeH;
}

uint16_t WifiMacHeader::GetFrameSize (void) const {
  return m_frameSize;
}
void WifiMacHeader::SetFrameSize (uint16_t frameSize) {
  m_frameSize = frameSize;
}

uint8_t WifiMacHeader::GetTA (void) const {
  return m_TA;
}
void WifiMacHeader::SetTA (uint8_t TA) {
  m_TA = TA;
}
...
uint32_t
WifiMacHeader::GetGmControl (void) const
{
  uint32_t val = 0;
  val |= (m_ctrlPktType) & (0x0f);
  val |= (m_nodeType << 4) & (0x03 << 4);
  val |= (m_groupNumber << 6) & (0x0f << 6);
  val |= (m_frameSize << 10) & (0x3ff << 10);
  val |= (m_TA << 20) & (0x3f << 20);
  return val;
}
void
WifiMacHeader::SetGmControl (uint32_t ctrl)
{
  m_ctrlPktType = (ctrl) & 0x0f;
  m_nodeType = (ctrl >> 4) & 0x03;
  m_groupNumber = (ctrl >> 6) & 0x0f;
  m_frameSize = (ctrl >> 10) & 0x3ff;
  m_TA = (ctrl >> 20) & 0x3f;
}
/**************************************************/
...
WifiMacHeader::GetSize (void) const
{
  ...
  switch (m_ctrlType)
    ...
    case TYPE_DATA:
      ...
      size += 4;    // GM-MAC GmControl field size (bytes)
      break;
    }
  return size;
}
