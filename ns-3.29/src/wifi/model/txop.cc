...
void
Txop::SetCurrentPacket(Ptr<const Packet> packet) {
...
{
  //NS_LOG_UNCOND("Txop::Queue");
  NS_LOG_FUNCTION (this << packet << &hdr);
  m_stationManager->PrepareForQueue (hdr.GetAddr1 (), &hdr, packet);
  m_queue->Enqueue (Create<WifiMacQueueItem> (packet, hdr));
  //NS_LOG_UNCOND("Txop::Queue header type checking"<< hdr.GetType());
  StartAccessIfNeeded ();
}
...
{
  //NS_LOG_UNCOND("Txop::RestartAccessIfNeeded");
  //Ptr<Node> node = m_stationManager->GetNode();
  //NS_LOG_UNCOND(Simulator::Now() << " id : " << node->GetId()+1 << " " <<m_currentPacket << " " << m_queue->IsEmpty() << " " << IsAccessRequested() << " " << m_low->IsCfPeriod());
  NS_LOG_FUNCTION (this);
...
{
  //NS_LOG_UNCOND("Txop::StartAccessIfNeeded");
  //Ptr<Node> node = m_stationManager->GetNode();
  //NS_LOG_UNCOND(Simulator::Now() << " id : " << node->GetId()+1 << " " <<m_currentPacket << " " << m_queue->IsEmpty() << " " << IsAccessRequested() << " " << m_low->IsCfPeriod());
  NS_LOG_FUNCTION (this);
  if (m_currentPacket == 0
...
Txop::NotifyAccessGranted (void)
{
  ...
  if (m_currentPacket == 0)
    {
      ...
      //NS_LOG_UNCOND("Txop::NotifyAccessGranted header type checking : " << m_currentHdr.GetType());
      m_fragmentNumber = 0;
  ...
  if (m_currentHdr.GetAddr1 ().IsGroup ())
    {
      //NS_LOG_UNCOND("Txop::NotifyAccessGranted / m_currentHdr.GetAddr1().IsGroup()");
...
{
  //NS_LOG_UNCOND("Txop::NotifyCollision");
  NS_LOG_FUNCTION (this);
  StartBackoffNow (m_rng->GetInteger (0, GetCw ()));
