...
{
  //NS_LOG_UNCOND("ChannelAccessManager::RequestAccess");
  NS_LOG_FUNCTION (this << state);
  ...
  UpdateBackoff ();   // update backoff slots
   ...
   */
  if (state->GetBackoffSlots () == 0)   // result of UpdateBackoff()
    {
      if (IsBusy ())
        {
  ...
  for (States::iterator i = m_states.begin (); i != m_states.end (); i++, k++)
    {
      ...
      Time backoffStart = GetBackoffStartFor (state);   // backoff start time
      if (backoffStart <= Simulator::Now ())
        {
          ...
          state->UpdateBackoffSlotsNow (n, backoffUpdateBound);   // Update backoff slots that nSlots has passed.
        }
    }
}
