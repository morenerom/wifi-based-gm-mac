...
void
MacLow::SetStateCallback (Callback<void, StateType> callback)
{
  m_stateCallback = callback;
}

void
MacLow::SetEnqueueCallback (Callback<void, Ptr<const Packet>, Mac48Address> callback)
{
  m_enqueueCallback = callback;
}

void
MacLow::CancelTACallback (Callback<void> callback)
{
  m_cancelTACallback = callback;
}
                           ...
                           Ptr<Txop> txop)
{
  ...
  //NS_LOG_UNCOND("MacLow::StartTransmission header type checking : " << m_currentHdr.GetType());
  if (m_txParams.MustSendRts ())
    {
      //if(hdr->GetCtrlPktType() == WIFI_MAC_GM_REPLY)
      //  NS_LOG_UNCOND("SendRtsForPacket REPLY " << packet->GetSize());
      SendRtsForPacket ();
    }
  else
    {
      if ((m_ctsToSelfSupported || m_stationManager->GetUseNonErpProtection ()) && NeedCtsToSelf ())
        ...
        {
          //NS_LOG_UNCOND("SendDataPacket without Rts");
          SendDataPacket ();
        }
    }
...
MacLow::CtsTimeout (void)
{
  ...
  m_currentTxop = 0;
  m_ampdu = false;
  if(node->GetId() != 0)  // sink does not regrouping
    m_ctsTimeoutCount += 1;
  if (m_ctsTimeoutCount < 7)
    txop->MissedCts ();
  else if (m_ctsTimeoutCount >= 7) {
    // Set currentPacket of Txop to 0
    txop->SetCurrentPacket(0);
    m_ctsTimeoutCount = 0;
    // Set Mac High State Type to REQ
    m_stateCallback(REQUEST);
    // Send a Request packets from Mac High
    Ptr<Packet> packet = Create<Packet> (0);
    Mac48Address m("ff:ff:ff:ff:ff:ff");
    m_enqueueCallback(packet, m);
  }
}
...
MacLow::SendDataPacket (void)
{
  ...
  if (m_ampdu)
  ...
  else
    {
      ...
      //NS_LOG_UNCOND("MacLow::SendDataPacket header type checking : " << m_currentHdr.GetType());
      AddWifiMacTrailer (packet);
    }
...
{
  //NS_LOG_UNCOND(Simulator::Now() << " MacLow::SendCtsAfterRts");
  Ptr<Node> node = m_stationManager->GetNode();
  //NS_LOG_UNCOND(node->GetId()+1 << " " << source);
