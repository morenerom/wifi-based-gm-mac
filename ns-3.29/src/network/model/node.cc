...
Node::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Node")
    ...
    .AddAttribute ("SystemId", "The systemId of this node: a unique integer used for parallel simulations.",
                   ...
                   MakeUintegerChecker<uint32_t> ())
    /*
    .AddTraceSource ("CurrentDataAmount",
                     "The amount of data in buffer.",
                     MakeTraceSourceAccessor (&Node::m_dataAmount),
    ...
    m_sid (0)
{
  m_dataAmount = 0;
  m_groupNumber = -1;
  m_bufferThreshold = 1024;
  m_nodeState = NODE_INIT;
  m_frameSize = 610;
  m_TA = 15;
...
vector<Data*> Node::GetDataBuffer (void) const {
  return m_dataBuffer;
}
void Node::SetDataBuffer (vector<Data*> dataBuffer) {
...
uint16_t Node::GetGroupNumber (void) const {
...
void Node::SetParentMacAddress (Mac48Address parentMacAddress) {
  m_parentMacAddress = parentMacAddress;
}
Mac48Address Node::GetParentMacAddress (void) {
  return m_parentMacAddress;
}

void Node::SetParentIpv4Address (Ipv4Address parentIpv4Address) {
  m_parentIpv4Address = parentIpv4Address;
}
Ipv4Address Node::GetParentIpv4Address (void) {
  return m_parentIpv4Address;
}

NodeState Node::GetNodeState (void) const {
  return m_nodeState;
}
...
uint16_t Node::GetFrameSize (void) const {
  return m_frameSize;
}
void Node::SetFrameSize (uint16_t frameSize) {
  m_frameSize = frameSize;
}

uint8_t Node::GetTA (void) const {
  return m_TA;
}
void Node::SetTA (uint8_t TA) {
  m_TA = TA;
}

void Node::Print (void) {
  uint16_t len = m_dataBuffer.size();
  for(uint16_t i=0;i<len;i++) {
    NS_LOG_UNCOND("Priority = " << (m_dataBuffer[i]->GetPriority() & 0xff));
    NS_LOG_UNCOND("DataType = " << (m_dataBuffer[i]->GetDataType() & 0xff));
    NS_LOG_UNCOND("NodeId = " << m_dataBuffer[i]->GetNodeId());
    NS_LOG_UNCOND("GenTime = " << Time(m_dataBuffer[i]->GetGenTime()));
  }
}
...
Node::AddDevice (Ptr<NetDevice> device)
{
  ...
  //device->SetReceiveGnCallback (MakeCallback (&Node::SetBufferThreshold, this));
...
{
  //NS_LOG_UNCOND("Node::ReceiveFromDevice");
