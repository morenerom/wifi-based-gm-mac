...
void 
Ipv4L3Protocol::SetCreateSocketCallback (Callback<void, Ptr<Node> > createSocket)
{
  m_createSocket = createSocket;
}
Ptr<Packet> Ipv4L3Protocol::GetCurPacket (void) {
  return m_curPacket;
}
...
{
  //NS_LOG_UNCOND("Ipv4L3Protocol::Receive");
  ...
  /*****************************************/
  // GM-MAC
  if(m_node->GetNodeState() == NODE_INIT) {
    m_node->SetParentIpv4Address(ipHeader.GetSource());
    m_node->SetNodeState(NODE_DATA);
    //NS_LOG_UNCOND("nodeId : " << m_node->GetId() << " / ParentIpv4Address : " << ipHeader.GetSource());
    Ptr<Ipv4> ip = m_node->GetObject<Ipv4> ();
    Ipv4Address src = ip->GetAddress(1, 0).GetLocal();     // sink ip address
    Ipv4Address dst = Ipv4Address::GetBroadcast ();
    Ptr<NetDevice> boundNetDevice = m_node->GetDevice(0);
    Ptr<Ipv4Route> route = Create<Ipv4Route> ();
    route->SetSource(src);
    route->SetDestination(dst);
    route->SetOutputDevice(boundNetDevice);
    m_createSocket(m_node);

    Send(Create<Packet> (0), src, dst, 0, route);
    return ;
  }
  /*****************************************/
...
Ipv4L3Protocol::Send (Ptr<Packet> packet, 
                      ...
                      Ptr<Ipv4Route> route)
{
  //NS_LOG_UNCOND("Ipv4L3Protocol::Send "<<packet->GetSize());
...
{
  //NS_LOG_UNCOND("Ipv4L3Protocol::SendRealOut" << packet->GetSize());
  ...
  if (!route->GetGateway ().IsEqual (Ipv4Address ("0.0.0.0")))
    {
      //NS_LOG_UNCOND("MTU size : " << outInterface->GetDevice ()->GetMtu ());
